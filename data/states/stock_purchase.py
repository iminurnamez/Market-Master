...
class StockPurchaseScreen(tools._State):
    def __init__(self):
        super(StockPurchaseScreen, self).__init__()
        ...
        self.title = Label(prepare.FONTS["weblysleekuisb"], 24, "Purchase Shares",
    ...
    def make_buttons(self):
        ...
        Button((b_left, b_top, bw, bh), self.buttons, text="Buy Shares", text_color=pg.Color("gray80"), 
                       fill_color=pg.Color("gray20"), font=self.font, font_size=14, call=self.buy_stock, click_sound=click)
    ...
    def buy_stock(self, *args):
        ...
        if not total:
            self.negative.play()
        elif self.player.cash >= total:
            self.player.portfolio.add_stock(self.stock, self.num_shares)
    ...
    def add_shares(self, num):
        if self.num_shares + num <= self.stock.num_shares:
            self.num_shares += num
    
    def subtract_shares(self, num):
        if num <= self.num_shares:
            self.num_shares -= num
        else:
            self.num_shares = 0
